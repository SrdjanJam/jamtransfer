...
require_once('lng/' . $_SESSION['CMSLang'] . '_text.php');
	...
	{		
		if (!LOCAL) {
			$DB_USER="taxifrom_transfersplitairport";
			$DB_PASSWORD=",D)DkaNPGe9K";
			$DB_NAME="taxifrom_transfersplitairport";
		...
		if($_REQUEST['username']!='' && ($_REQUEST['password']!='' || $_REQUEST['tempPass']!=''))
		{
			...
			if($result->num_rows == 1)
			{
				...
				if($row['Active'] == 1)
				{
					...
					if (!in_array($_SESSION['AuthLevelID'],$levels)) saveLog($row['AuthUserID'],1);
