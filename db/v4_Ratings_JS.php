...
<script type="text/javascript">
	...
	function all_v4_Ratings() {
		...
		var callFunction = 'all_v4_RatingsFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Ratings/v4_Ratings_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_RatingsData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_RatingsData.data;
			  recordsTotal = v4_RatingsData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Ratings = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_RatingsListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Ratings : data_v4_Ratings});

				$("#show_v4_Ratings").html(HTML);
		  },
	...
	function one_v4_Ratings(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_RatingsWrapper"+id).css('display') != 'none') {$("#v4_RatingsWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Ratings/v4_Ratings_One.php?OwnerID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_RatingsEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Ratings"+id).html(HTML);

				$("#v4_RatingsWrapper"+id).show('slow');
			},
	...
	function new_v4_Ratings() { 
		var data = {
				...
				$("#new_v4_Ratings").html(HTML);

				$("#v4_RatingsWrapperNew").show('slow');

	}


	function editClosev4_Ratings(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Ratings(id, inList) { 
	
		if($("#v4_RatingsEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_RatingsEditForm"+id).serializeObject();
		var formData = $("#v4_RatingsEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Ratings/'+
		'v4_Ratings_Save.php?callback=?&keyName=OwnerID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_RatingsData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Ratings(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Ratings/'+
		'v4_Ratings_Delete.php?OwnerID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Ratings(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_RatingsData(id, newData) {

	  var result = $.grep(data_v4_Ratings, function(e){ return e.OwnerID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Ratings(id, ress);

		    data = data_v4_Ratings;

		    var source   = $("#v4_RatingsListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Ratings : data});
	...
	function changeall_v4_Ratings( id, sve ) {
	   for (var i in data_v4_Ratings) {
		 if (data_v4_Ratings[i].OwnerID == id) {
		    data_v4_Ratings[i] = sve;
