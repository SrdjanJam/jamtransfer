...
<script type="text/javascript">
	...
	var data_v4_Settings = [];
	function all_v4_Settings() {
		...
		var callFunction = 'all_v4_SettingsFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Settings/v4_Settings_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_SettingsData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_SettingsData.data;
			  recordsTotal = v4_SettingsData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Settings = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_SettingsListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Settings : data_v4_Settings});

				$("#show_v4_Settings").html(HTML);
		  },
	...
	function one_v4_Settings(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_SettingsWrapper"+id).css('display') != 'none') {$("#v4_SettingsWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Settings/v4_Settings_One.php?id='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_SettingsEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Settings"+id).html(HTML);

				$("#v4_SettingsWrapper"+id).show('slow');
			},
	...
	function new_v4_Settings() { 
		var data = {
				...
				$("#new_v4_Settings").html(HTML);

				$("#v4_SettingsWrapperNew").show('slow');

	}


	function editClosev4_Settings(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Settings(id, inList) { 
	
		if($("#v4_SettingsEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_SettingsEditForm"+id).serializeObject();
		var formData = $("#v4_SettingsEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Settings/'+
		'v4_Settings_Save.php?callback=?&keyName=id&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_SettingsData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Settings(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Settings/'+
		'v4_Settings_Delete.php?id='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Settings(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_SettingsData(id, newData) {

	  var result = $.grep(data_v4_Settings, function(e){ return e.id == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Settings(id, ress);

		    data = data_v4_Settings;

		    var source   = $("#v4_SettingsListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Settings : data});
	...
	function changeall_v4_Settings( id, sve ) {
	   for (var i in data_v4_Settings) {
		 if (data_v4_Settings[i].id == id) {
		    data_v4_Settings[i] = sve;
