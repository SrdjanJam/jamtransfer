...
<script type="text/javascript">
	...
	var data_v4_Worksheet = [];
	function all_v4_Worksheet() {
		...
		var callFunction = 'all_v4_WorksheetFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Worksheet/v4_Worksheet_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_WorksheetData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_WorksheetData.data;
			  recordsTotal = v4_WorksheetData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Worksheet = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_WorksheetListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Worksheet : data_v4_Worksheet});

				$("#show_v4_Worksheet").html(HTML);
		  },
	...
	function one_v4_Worksheet(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_WorksheetWrapper"+id).css('display') != 'none') {$("#v4_WorksheetWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Worksheet/v4_Worksheet_One.php?WSID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_WorksheetEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Worksheet"+id).html(HTML);

				$("#v4_WorksheetWrapper"+id).show('slow');
			},
	...
	function new_v4_Worksheet() { 
		var data = {
				...
				$("#new_v4_Worksheet").html(HTML);

				$("#v4_WorksheetWrapperNew").show('slow');

	}


	function editClosev4_Worksheet(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Worksheet(id, inList) { 
	
		if($("#v4_WorksheetEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_WorksheetEditForm"+id).serializeObject();
		var formData = $("#v4_WorksheetEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Worksheet/'+
		'v4_Worksheet_Save.php?callback=?&keyName=WSID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_WorksheetData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Worksheet(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Worksheet/'+
		'v4_Worksheet_Delete.php?WSID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Worksheet(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_WorksheetData(id, newData) {

	  var result = $.grep(data_v4_Worksheet, function(e){ return e.WSID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Worksheet(id, ress);

		    data = data_v4_Worksheet;

		    var source   = $("#v4_WorksheetListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Worksheet : data});
	...
	function changeall_v4_Worksheet( id, sve ) {
	   for (var i in data_v4_Worksheet) {
		 if (data_v4_Worksheet[i].WSID == id) {
		    data_v4_Worksheet[i] = sve;
