...
<script type="text/javascript">
	...
	var data_v4_Surcharges = [];
	function all_v4_Surcharges() {
		...
		var callFunction = 'all_v4_SurchargesFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Surcharges/v4_Surcharges_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_SurchargesData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_SurchargesData.data;
			  recordsTotal = v4_SurchargesData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Surcharges = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_SurchargesListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Surcharges : data_v4_Surcharges});

				$("#show_v4_Surcharges").html(HTML);
		  },
	...
	function one_v4_Surcharges(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_SurchargesWrapper"+id).css('display') != 'none') {$("#v4_SurchargesWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Surcharges/v4_Surcharges_One.php?OwnerID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_SurchargesEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Surcharges"+id).html(HTML);

				$("#v4_SurchargesWrapper"+id).show('slow');
			},
	...
	function new_v4_Surcharges() { 
		var data = {
				...
				$("#new_v4_Surcharges").html(HTML);

				$("#v4_SurchargesWrapperNew").show('slow');

	}


	function editClosev4_Surcharges(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Surcharges(id, inList) { 
	
		if($("#v4_SurchargesEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_SurchargesEditForm"+id).serializeObject();
		var formData = $("#v4_SurchargesEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Surcharges/'+
		'v4_Surcharges_Save.php?callback=?&keyName=OwnerID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_SurchargesData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Surcharges(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Surcharges/'+
		'v4_Surcharges_Delete.php?OwnerID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Surcharges(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_SurchargesData(id, newData) {

	  var result = $.grep(data_v4_Surcharges, function(e){ return e.OwnerID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Surcharges(id, ress);

		    data = data_v4_Surcharges;

		    var source   = $("#v4_SurchargesListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Surcharges : data});
	...
	function changeall_v4_Surcharges( id, sve ) {
	   for (var i in data_v4_Surcharges) {
		 if (data_v4_Surcharges[i].OwnerID == id) {
		    data_v4_Surcharges[i] = sve;
