...
<script type="text/javascript">
	...
	var data_v4_Messages = [];
	function all_v4_Messages() {
		...
		var callFunction = 'all_v4_MessagesFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Messages/v4_Messages_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_MessagesData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_MessagesData.data;
			  recordsTotal = v4_MessagesData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Messages = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_MessagesListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Messages : data_v4_Messages});

				$("#show_v4_Messages").html(HTML);
		  },
	...
	function one_v4_Messages(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_MessagesWrapper"+id).css('display') != 'none') {$("#v4_MessagesWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Messages/v4_Messages_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_MessagesEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Messages"+id).html(HTML);

				$("#v4_MessagesWrapper"+id).show('slow');
			},
	...
	function new_v4_Messages() { 
		var data = {
				...
				$("#new_v4_Messages").html(HTML);

				$("#v4_MessagesWrapperNew").show('slow');

	}


	function editClosev4_Messages(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Messages(id, inList) { 
	
		if($("#v4_MessagesEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_MessagesEditForm"+id).serializeObject();
		var formData = $("#v4_MessagesEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Messages/'+
		'v4_Messages_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_MessagesData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Messages(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Messages/'+
		'v4_Messages_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Messages(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_MessagesData(id, newData) {

	  var result = $.grep(data_v4_Messages, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Messages(id, ress);

		    data = data_v4_Messages;

		    var source   = $("#v4_MessagesListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Messages : data});
	...
	function changeall_v4_Messages( id, sve ) {
	   for (var i in data_v4_Messages) {
		 if (data_v4_Messages[i].ID == id) {
		    data_v4_Messages[i] = sve;
