...
<script type="text/javascript">
	...
	function all_v4_yes_no() {
		...
		var callFunction = 'all_v4_yes_noFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_yes_no/v4_yes_no_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_yes_noData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_yes_noData.data;
			  recordsTotal = v4_yes_noData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				var source   = $("#v4_yes_noListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_yes_no : data_v4_yes_no});

				$("#show_v4_yes_no").html(HTML);
		  },
	...
	function one_v4_yes_no(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_yes_noWrapper"+id).css('display') != 'none') {$("#v4_yes_noWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_yes_no/v4_yes_no_One.php?dn_broj='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_yes_noEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_yes_no"+id).html(HTML);

				$("#v4_yes_noWrapper"+id).show('slow');
			},
	...
	function new_v4_yes_no() { 
		var data = {
				...
				$("#new_v4_yes_no").html(HTML);

				$("#v4_yes_noWrapperNew").show('slow');

	}


	function editClosev4_yes_no(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_yes_no(id, inList) { 
	
		if($("#v4_yes_noEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_yes_noEditForm"+id).serializeObject();
		var formData = $("#v4_yes_noEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_yes_no/'+
		'v4_yes_no_Save.php?callback=?&keyName=dn_broj&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_yes_noData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_yes_no(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_yes_no/'+
		'v4_yes_no_Delete.php?dn_broj='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_yes_no(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_yes_noData(id, newData) {

	  var result = $.grep(data_v4_yes_no, function(e){ return e.dn_broj == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    var HTML = template({v4_yes_no : data});
	...
	function changeall_v4_yes_no( id, sve ) {
	   for (var i in data_v4_yes_no) {
		 if (data_v4_yes_no[i].dn_broj == id) {
		    data_v4_yes_no[i] = sve;
