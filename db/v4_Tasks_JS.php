...
<script type="text/javascript">
	...
	function all_v4_Tasks() {
		...
		var callFunction = 'all_v4_TasksFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Tasks/v4_Tasks_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_TasksData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_TasksData.data;
			  recordsTotal = v4_TasksData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				var source   = $("#v4_TasksListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Tasks : data_v4_Tasks});

				$("#show_v4_Tasks").html(HTML);
		  },
	...
	function one_v4_Tasks(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_TasksWrapper"+id).css('display') != 'none') {$("#v4_TasksWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Tasks/v4_Tasks_One.php?TaskID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_TasksEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Tasks"+id).html(HTML);

				$("#v4_TasksWrapper"+id).show('slow');
			},
	...
	function new_v4_Tasks() { 
		var data = {
				...
				$("#new_v4_Tasks").html(HTML);

				$("#v4_TasksWrapperNew").show('slow');

	}


	function editClosev4_Tasks(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Tasks(id, inList) { 
	
		if($("#v4_TasksEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_TasksEditForm"+id).serializeObject();
		var formData = $("#v4_TasksEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Tasks/'+
		'v4_Tasks_Save.php?callback=?&keyName=TaskID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_TasksData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Tasks(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Tasks/'+
		'v4_Tasks_Delete.php?TaskID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Tasks(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_TasksData(id, newData) {

	  var result = $.grep(data_v4_Tasks, function(e){ return e.TaskID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    var HTML = template({v4_Tasks : data});
	...
	function changeall_v4_Tasks( id, sve ) {
	   for (var i in data_v4_Tasks) {
		 if (data_v4_Tasks[i].TaskID == id) {
		    data_v4_Tasks[i] = sve;
