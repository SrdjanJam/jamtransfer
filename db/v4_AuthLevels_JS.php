...
<script type="text/javascript">
	...
	var data_v4_AuthLevels = [];
	function all_v4_AuthLevels() {
		...
		var callFunction = 'all_v4_AuthLevelsFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_AuthLevels/v4_AuthLevels_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_AuthLevelsData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_AuthLevelsData.data;
			  recordsTotal = v4_AuthLevelsData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_AuthLevels = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_AuthLevelsListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_AuthLevels : data_v4_AuthLevels});

				$("#show_v4_AuthLevels").html(HTML);
		  },
	...
	function one_v4_AuthLevels(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_AuthLevelsWrapper"+id).css('display') != 'none') {$("#v4_AuthLevelsWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_AuthLevels/v4_AuthLevels_One.php?AuthLevelID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_AuthLevelsEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_AuthLevels"+id).html(HTML);

				$("#v4_AuthLevelsWrapper"+id).show('slow');
			},
	...
	function new_v4_AuthLevels() { 
		var data = {
				...
				$("#new_v4_AuthLevels").html(HTML);

				$("#v4_AuthLevelsWrapperNew").show('slow');

	}


	function editClosev4_AuthLevels(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_AuthLevels(id, inList) { 
	
		if($("#v4_AuthLevelsEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_AuthLevelsEditForm"+id).serializeObject();
		var formData = $("#v4_AuthLevelsEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_AuthLevels/'+
		'v4_AuthLevels_Save.php?callback=?&keyName=AuthLevelID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_AuthLevelsData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_AuthLevels(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_AuthLevels/'+
		'v4_AuthLevels_Delete.php?AuthLevelID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_AuthLevels(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_AuthLevelsData(id, newData) {

	  var result = $.grep(data_v4_AuthLevels, function(e){ return e.AuthLevelID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_AuthLevels(id, ress);

		    data = data_v4_AuthLevels;

		    var source   = $("#v4_AuthLevelsListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_AuthLevels : data});
	...
	function changeall_v4_AuthLevels( id, sve ) {
	   for (var i in data_v4_AuthLevels) {
		 if (data_v4_AuthLevels[i].AuthLevelID == id) {
		    data_v4_AuthLevels[i] = sve;
