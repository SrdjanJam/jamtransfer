...
<script type="text/javascript">
	...
	function all_v4_x() {
		...
		var callFunction = 'all_v4_xFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_x/v4_x_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_xData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_xData.data;
			  recordsTotal = v4_xData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				var source   = $("#v4_xListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_x : data_v4_x});
	...
	function one_v4_x(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_xWrapper"+id).css('display') != 'none') {$("#v4_xWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_x/v4_x_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_xEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_x"+id).html(HTML);

				$("#v4_xWrapper"+id).show('slow');
			},
	...
	function new_v4_x() { 
		var data = {
				...
				$("#v4_xWrapperNew").show('slow');

	}


	function editClosev4_x(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_x(id, inList) { 
	
		if($("#v4_xEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_xEditForm"+id).serializeObject();
		var formData = $("#v4_xEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_x/'+
		'v4_x_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
	...
	function deletev4_x(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_x/'+
		'v4_x_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_x(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_xData(id, newData) {

	  var result = $.grep(data_v4_x, function(e){ return e.ID == id; });
	...
	function changeall_v4_x( id, sve ) {
	   for (var i in data_v4_x) {
		 if (data_v4_x[i].ID == id) {
		    data_v4_x[i] = sve;
