...
<script type="text/javascript">
	...
	var data_v4_Invoices = [];
	function all_v4_Invoices() {
		...
		var callFunction = 'all_v4_InvoicesFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Invoices/v4_Invoices_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_InvoicesData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_InvoicesData.data;
			  recordsTotal = v4_InvoicesData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Invoices = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_InvoicesListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Invoices : data_v4_Invoices});

				$("#show_v4_Invoices").html(HTML);
		  },
	...
	function one_v4_Invoices(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_InvoicesWrapper"+id).css('display') != 'none') {$("#v4_InvoicesWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Invoices/v4_Invoices_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_InvoicesEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Invoices"+id).html(HTML);

				$("#v4_InvoicesWrapper"+id).show('slow');
			},
	...
	function new_v4_Invoices() { 
		var data = {
				...
				$("#new_v4_Invoices").html(HTML);

				$("#v4_InvoicesWrapperNew").show('slow');

	}


	function editClosev4_Invoices(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Invoices(id, inList) { 
	
		if($("#v4_InvoicesEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_InvoicesEditForm"+id).serializeObject();
		var formData = $("#v4_InvoicesEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Invoices/'+
		'v4_Invoices_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_InvoicesData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Invoices(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Invoices/'+
		'v4_Invoices_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Invoices(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_InvoicesData(id, newData) {

	  var result = $.grep(data_v4_Invoices, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Invoices(id, ress);

		    data = data_v4_Invoices;

		    var source   = $("#v4_InvoicesListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Invoices : data});
	...
	function changeall_v4_Invoices( id, sve ) {
	   for (var i in data_v4_Invoices) {
		 if (data_v4_Invoices[i].ID == id) {
		    data_v4_Invoices[i] = sve;
