...
<script type="text/javascript">
	...
	var data_v4_InvoicesMaster = [];
	function all_v4_InvoicesMaster() {
		...
		var callFunction = 'all_v4_InvoicesMasterFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_InvoicesMaster/v4_InvoicesMaster_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_InvoicesMasterData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_InvoicesMasterData.data;
			  recordsTotal = v4_InvoicesMasterData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_InvoicesMaster = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_InvoicesMasterListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_InvoicesMaster : data_v4_InvoicesMaster});

				$("#show_v4_InvoicesMaster").html(HTML);
		  },
	...
	function one_v4_InvoicesMaster(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_InvoicesMasterWrapper"+id).css('display') != 'none') {$("#v4_InvoicesMasterWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_InvoicesMaster/v4_InvoicesMaster_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_InvoicesMasterEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_InvoicesMaster"+id).html(HTML);

				$("#v4_InvoicesMasterWrapper"+id).show('slow');
			},
	...
	function new_v4_InvoicesMaster() { 
		var data = {
				...
				$("#new_v4_InvoicesMaster").html(HTML);

				$("#v4_InvoicesMasterWrapperNew").show('slow');

	}


	function editClosev4_InvoicesMaster(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_InvoicesMaster(id, inList) { 
	
		if($("#v4_InvoicesMasterEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_InvoicesMasterEditForm"+id).serializeObject();
		var formData = $("#v4_InvoicesMasterEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_InvoicesMaster/'+
		'v4_InvoicesMaster_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_InvoicesMasterData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_InvoicesMaster(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_InvoicesMaster/'+
		'v4_InvoicesMaster_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_InvoicesMaster(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_InvoicesMasterData(id, newData) {

	  var result = $.grep(data_v4_InvoicesMaster, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_InvoicesMaster(id, ress);

		    data = data_v4_InvoicesMaster;

		    var source   = $("#v4_InvoicesMasterListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_InvoicesMaster : data});
	...
	function changeall_v4_InvoicesMaster( id, sve ) {
	   for (var i in data_v4_InvoicesMaster) {
		 if (data_v4_InvoicesMaster[i].ID == id) {
		    data_v4_InvoicesMaster[i] = sve;
