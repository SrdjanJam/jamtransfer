...
<script type="text/javascript">
	...
	var data_v4_Customers = [];
	function all_v4_Customers() {
		...
		var callFunction = 'all_v4_CustomersFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Customers/v4_Customers_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_CustomersData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_CustomersData.data;
			  recordsTotal = v4_CustomersData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Customers = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_CustomersListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Customers : data_v4_Customers});

				$("#show_v4_Customers").html(HTML);
		  },
	...
	function one_v4_Customers(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_CustomersWrapper"+id).css('display') != 'none') {$("#v4_CustomersWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Customers/v4_Customers_One.php?CustID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_CustomersEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Customers"+id).html(HTML);

				$("#v4_CustomersWrapper"+id).show('slow');
			},
	...
	function new_v4_Customers() { 
		var data = {
				...
				$("#new_v4_Customers").html(HTML);

				$("#v4_CustomersWrapperNew").show('slow');

	}


	function editClosev4_Customers(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Customers(id, inList) { 
	
		if($("#v4_CustomersEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_CustomersEditForm"+id).serializeObject();
		var formData = $("#v4_CustomersEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Customers/'+
		'v4_Customers_Save.php?callback=?&keyName=CustID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_CustomersData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Customers(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Customers/'+
		'v4_Customers_Delete.php?CustID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Customers(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_CustomersData(id, newData) {

	  var result = $.grep(data_v4_Customers, function(e){ return e.CustID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Customers(id, ress);

		    data = data_v4_Customers;

		    var source   = $("#v4_CustomersListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Customers : data});
	...
	function changeall_v4_Customers( id, sve ) {
	   for (var i in data_v4_Customers) {
		 if (data_v4_Customers[i].CustID == id) {
		    data_v4_Customers[i] = sve;
