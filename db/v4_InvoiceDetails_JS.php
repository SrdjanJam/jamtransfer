...
<script type="text/javascript">
	...
	var data_v4_InvoiceDetails = [];
	function all_v4_InvoiceDetails() {
		...
		var callFunction = 'all_v4_InvoiceDetailsFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_InvoiceDetails/v4_InvoiceDetails_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_InvoiceDetailsData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_InvoiceDetailsData.data;
			  recordsTotal = v4_InvoiceDetailsData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_InvoiceDetails = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_InvoiceDetailsListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_InvoiceDetails : data_v4_InvoiceDetails});

				$("#show_v4_InvoiceDetails").html(HTML);
		  },
	...
	function one_v4_InvoiceDetails(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_InvoiceDetailsWrapper"+id).css('display') != 'none') {$("#v4_InvoiceDetailsWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_InvoiceDetails/v4_InvoiceDetails_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_InvoiceDetailsEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_InvoiceDetails"+id).html(HTML);

				$("#v4_InvoiceDetailsWrapper"+id).show('slow');
			},
	...
	function new_v4_InvoiceDetails() { 
		var data = {
				...
				$("#new_v4_InvoiceDetails").html(HTML);

				$("#v4_InvoiceDetailsWrapperNew").show('slow');

	}


	function editClosev4_InvoiceDetails(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_InvoiceDetails(id, inList) { 
	
		if($("#v4_InvoiceDetailsEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_InvoiceDetailsEditForm"+id).serializeObject();
		var formData = $("#v4_InvoiceDetailsEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_InvoiceDetails/'+
		'v4_InvoiceDetails_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_InvoiceDetailsData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_InvoiceDetails(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_InvoiceDetails/'+
		'v4_InvoiceDetails_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_InvoiceDetails(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_InvoiceDetailsData(id, newData) {

	  var result = $.grep(data_v4_InvoiceDetails, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_InvoiceDetails(id, ress);

		    data = data_v4_InvoiceDetails;

		    var source   = $("#v4_InvoiceDetailsListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_InvoiceDetails : data});
	...
	function changeall_v4_InvoiceDetails( id, sve ) {
	   for (var i in data_v4_InvoiceDetails) {
		 if (data_v4_InvoiceDetails[i].ID == id) {
		    data_v4_InvoiceDetails[i] = sve;
