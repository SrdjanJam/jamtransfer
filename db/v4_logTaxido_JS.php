...
<script type="text/javascript">
	...
	var data_v4_logTaxido = [];
	function all_v4_logTaxido() {
		...
		var callFunction = 'all_v4_logTaxidoFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_logTaxido/v4_logTaxido_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_logTaxidoData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_logTaxidoData.data;
			  recordsTotal = v4_logTaxidoData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_logTaxido = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_logTaxidoListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_logTaxido : data_v4_logTaxido});

				$("#show_v4_logTaxido").html(HTML);
		  },
	...
	function one_v4_logTaxido(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_logTaxidoWrapper"+id).css('display') != 'none') {$("#v4_logTaxidoWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_logTaxido/v4_logTaxido_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_logTaxidoEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_logTaxido"+id).html(HTML);

				$("#v4_logTaxidoWrapper"+id).show('slow');
			},
	...
	function new_v4_logTaxido() { 
		var data = {
				...
				$("#new_v4_logTaxido").html(HTML);

				$("#v4_logTaxidoWrapperNew").show('slow');

	}


	function editClosev4_logTaxido(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_logTaxido(id, inList) { 
	
		if($("#v4_logTaxidoEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_logTaxidoEditForm"+id).serializeObject();
		var formData = $("#v4_logTaxidoEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_logTaxido/'+
		'v4_logTaxido_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_logTaxidoData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_logTaxido(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_logTaxido/'+
		'v4_logTaxido_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_logTaxido(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_logTaxidoData(id, newData) {

	  var result = $.grep(data_v4_logTaxido, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_logTaxido(id, ress);

		    data = data_v4_logTaxido;

		    var source   = $("#v4_logTaxidoListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_logTaxido : data});
	...
	function changeall_v4_logTaxido( id, sve ) {
	   for (var i in data_v4_logTaxido) {
		 if (data_v4_logTaxido[i].ID == id) {
		    data_v4_logTaxido[i] = sve;
