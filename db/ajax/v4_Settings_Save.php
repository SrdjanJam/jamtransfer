...
error_reporting(E_PARSE);
	...
	require_once '../../../../db/v4_Settings.class.php';
...
if ($keyName != '' and $keyValue != '') $db->getRow($keyValue);
	...
	if(isset($_REQUEST['setkey'])) { $db->setsetkey($db->myreal_escape_string($_REQUEST['setkey']) ); } 

		 	
	if(isset($_REQUEST['setval'])) { $db->setsetval($db->myreal_escape_string($_REQUEST['setval']) ); } 

		 	
	if(isset($_REQUEST['settype'])) { $db->setsettype($db->myreal_escape_string($_REQUEST['settype']) ); } 
