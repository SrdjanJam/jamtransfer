...
if ($keyName != '' and $keyValue != '') $db->getRow($keyValue);
	...
	if(isset($_REQUEST['MsgFrom'])) { $db->setMsgFrom($db->myreal_escape_string($_REQUEST['MsgFrom']) ); } 

		 	
	if(isset($_REQUEST['FromName'])) { $db->setFromName($db->myreal_escape_string($_REQUEST['FromName']) ); } 

		 	
	if(isset($_REQUEST['Msg'])) { $db->setMsg($db->myreal_escape_string($_REQUEST['Msg']) ); } 

		 	
	if(isset($_REQUEST['Body'])) { $db->setBody($db->myreal_escape_string($_REQUEST['Body']) ); } 

		 	
	if(isset($_REQUEST['UserID'])) { $db->setUserID($db->myreal_escape_string($_REQUEST['UserID']) ); } 

		 	
	if(isset($_REQUEST['DateTime'])) { $db->setDateTime($db->myreal_escape_string($_REQUEST['DateTime']) ); } 

		 	
	if(isset($_REQUEST['UserLevel'])) { $db->setUserLevel($db->myreal_escape_string($_REQUEST['UserLevel']) ); } 
