...
<script type="text/javascript">
	...
	var data_v4_PaymentGateways = [];
	function all_v4_PaymentGateways() {
		...
		var callFunction = 'all_v4_PaymentGatewaysFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_PaymentGateways/v4_PaymentGateways_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_PaymentGatewaysData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_PaymentGatewaysData.data;
			  recordsTotal = v4_PaymentGatewaysData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_PaymentGateways = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_PaymentGatewaysListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_PaymentGateways : data_v4_PaymentGateways});

				$("#show_v4_PaymentGateways").html(HTML);
		  },
	...
	function one_v4_PaymentGateways(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_PaymentGatewaysWrapper"+id).css('display') != 'none') {$("#v4_PaymentGatewaysWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_PaymentGateways/v4_PaymentGateways_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_PaymentGatewaysEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_PaymentGateways"+id).html(HTML);

				$("#v4_PaymentGatewaysWrapper"+id).show('slow');
			},
	...
	function new_v4_PaymentGateways() { 
		var data = {
				...
				$("#new_v4_PaymentGateways").html(HTML);

				$("#v4_PaymentGatewaysWrapperNew").show('slow');

	}


	function editClosev4_PaymentGateways(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_PaymentGateways(id, inList) { 
	
		if($("#v4_PaymentGatewaysEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_PaymentGatewaysEditForm"+id).serializeObject();
		var formData = $("#v4_PaymentGatewaysEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_PaymentGateways/'+
		'v4_PaymentGateways_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_PaymentGatewaysData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_PaymentGateways(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_PaymentGateways/'+
		'v4_PaymentGateways_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_PaymentGateways(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_PaymentGatewaysData(id, newData) {

	  var result = $.grep(data_v4_PaymentGateways, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_PaymentGateways(id, ress);

		    data = data_v4_PaymentGateways;

		    var source   = $("#v4_PaymentGatewaysListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_PaymentGateways : data});
	...
	function changeall_v4_PaymentGateways( id, sve ) {
	   for (var i in data_v4_PaymentGateways) {
		 if (data_v4_PaymentGateways[i].ID == id) {
		    data_v4_PaymentGateways[i] = sve;
