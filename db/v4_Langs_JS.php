...
<script type="text/javascript">
	...
	function all_v4_Langs() {
		...
		var callFunction = 'all_v4_LangsFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Langs/v4_Langs_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_LangsData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_LangsData.data;
			  recordsTotal = v4_LangsData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				var source   = $("#v4_LangsListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Langs : data_v4_Langs});

				$("#show_v4_Langs").html(HTML);
		  },
	...
	function one_v4_Langs(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_LangsWrapper"+id).css('display') != 'none') {$("#v4_LangsWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Langs/v4_Langs_One.php?lang_short='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_LangsEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Langs"+id).html(HTML);

				$("#v4_LangsWrapper"+id).show('slow');
			},
	...
	function new_v4_Langs() { 
		var data = {
				...
				$("#new_v4_Langs").html(HTML);

				$("#v4_LangsWrapperNew").show('slow');

	}


	function editClosev4_Langs(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Langs(id, inList) { 
	
		if($("#v4_LangsEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_LangsEditForm"+id).serializeObject();
		var formData = $("#v4_LangsEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Langs/'+
		'v4_Langs_Save.php?callback=?&keyName=lang_short&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_LangsData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Langs(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Langs/'+
		'v4_Langs_Delete.php?lang_short='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Langs(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_LangsData(id, newData) {

	  var result = $.grep(data_v4_Langs, function(e){ return e.lang_short == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    var HTML = template({v4_Langs : data});
	...
	function changeall_v4_Langs( id, sve ) {
	   for (var i in data_v4_Langs) {
		 if (data_v4_Langs[i].lang_short == id) {
		    data_v4_Langs[i] = sve;
