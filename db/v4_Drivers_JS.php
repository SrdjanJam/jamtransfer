...
<script type="text/javascript">
	...
	function all_v4_Drivers() {
		...
		var callFunction = 'all_v4_DriversFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Drivers/v4_Drivers_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_DriversData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_DriversData.data;
			  recordsTotal = v4_DriversData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Drivers = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_DriversListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Drivers : data_v4_Drivers});

				$("#show_v4_Drivers").html(HTML);
		  },
	...
	function one_v4_Drivers(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_DriversWrapper"+id).css('display') != 'none') {$("#v4_DriversWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Drivers/v4_Drivers_One.php?DriverID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_DriversEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Drivers"+id).html(HTML);

				$("#v4_DriversWrapper"+id).show('slow');
			},
	...
	function new_v4_Drivers() { 
		var data = {
				...
				$("#new_v4_Drivers").html(HTML);

				$("#v4_DriversWrapperNew").show('slow');

	}


	function editClosev4_Drivers(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Drivers(id, inList) { 
	
		if($("#v4_DriversEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_DriversEditForm"+id).serializeObject();
		var formData = $("#v4_DriversEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Drivers/'+
		'v4_Drivers_Save.php?callback=?&keyName=DriverID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_DriversData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Drivers(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Drivers/'+
		'v4_Drivers_Delete.php?DriverID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Drivers(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_DriversData(id, newData) {

	  var result = $.grep(data_v4_Drivers, function(e){ return e.DriverID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Drivers(id, ress);

		    data = data_v4_Drivers;

		    var source   = $("#v4_DriversListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Drivers : data});
	...
	function changeall_v4_Drivers( id, sve ) {
	   for (var i in data_v4_Drivers) {
		 if (data_v4_Drivers[i].DriverID == id) {
		    data_v4_Drivers[i] = sve;
