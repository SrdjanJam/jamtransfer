...
<script type="text/javascript">
	...
	var data_v4_OrderStatuses = [];
	function all_v4_OrderStatuses() {
		...
		var callFunction = 'all_v4_OrderStatusesFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_OrderStatuses/v4_OrderStatuses_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_OrderStatusesData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_OrderStatusesData.data;
			  recordsTotal = v4_OrderStatusesData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_OrderStatuses = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_OrderStatusesListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_OrderStatuses : data_v4_OrderStatuses});

				$("#show_v4_OrderStatuses").html(HTML);
		  },
	...
	function one_v4_OrderStatuses(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_OrderStatusesWrapper"+id).css('display') != 'none') {$("#v4_OrderStatusesWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_OrderStatuses/v4_OrderStatuses_One.php?OrderStatusID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_OrderStatusesEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_OrderStatuses"+id).html(HTML);

				$("#v4_OrderStatusesWrapper"+id).show('slow');
			},
	...
	function new_v4_OrderStatuses() { 
		var data = {
				...
				$("#new_v4_OrderStatuses").html(HTML);

				$("#v4_OrderStatusesWrapperNew").show('slow');

	}


	function editClosev4_OrderStatuses(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_OrderStatuses(id, inList) { 
	
		if($("#v4_OrderStatusesEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_OrderStatusesEditForm"+id).serializeObject();
		var formData = $("#v4_OrderStatusesEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_OrderStatuses/'+
		'v4_OrderStatuses_Save.php?callback=?&keyName=OrderStatusID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_OrderStatusesData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_OrderStatuses(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_OrderStatuses/'+
		'v4_OrderStatuses_Delete.php?OrderStatusID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_OrderStatuses(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_OrderStatusesData(id, newData) {

	  var result = $.grep(data_v4_OrderStatuses, function(e){ return e.OrderStatusID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_OrderStatuses(id, ress);

		    data = data_v4_OrderStatuses;

		    var source   = $("#v4_OrderStatusesListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_OrderStatuses : data});
	...
	function changeall_v4_OrderStatuses( id, sve ) {
	   for (var i in data_v4_OrderStatuses) {
		 if (data_v4_OrderStatuses[i].OrderStatusID == id) {
		    data_v4_OrderStatuses[i] = sve;
