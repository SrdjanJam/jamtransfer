...
<script type="text/javascript">
	...
	function all_v4_Extras() {
		...
		var callFunction = 'all_v4_ExtrasFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Extras/v4_Extras_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_ExtrasData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_ExtrasData.data;
			  recordsTotal = v4_ExtrasData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				var source   = $("#v4_ExtrasListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Extras : data_v4_Extras});

				$("#show_v4_Extras").html(HTML);
		  },
	...
	function one_v4_Extras(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_ExtrasWrapper"+id).css('display') != 'none') {$("#v4_ExtrasWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Extras/v4_Extras_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_ExtrasEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Extras"+id).html(HTML);

				$("#v4_ExtrasWrapper"+id).show('slow');
			},
	...
	function new_v4_Extras() { 
		var data = {
				...
				$("#new_v4_Extras").html(HTML);

				$("#v4_ExtrasWrapperNew").show('slow');

	}


	function editClosev4_Extras(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Extras(id, inList) { 
	
		if($("#v4_ExtrasEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_ExtrasEditForm"+id).serializeObject();
		var formData = $("#v4_ExtrasEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Extras/'+
		'v4_Extras_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_ExtrasData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Extras(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Extras/'+
		'v4_Extras_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Extras(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_ExtrasData(id, newData) {

	  var result = $.grep(data_v4_Extras, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    var HTML = template({v4_Extras : data});
	...
	function changeall_v4_Extras( id, sve ) {
	   for (var i in data_v4_Extras) {
		 if (data_v4_Extras[i].ID == id) {
		    data_v4_Extras[i] = sve;
