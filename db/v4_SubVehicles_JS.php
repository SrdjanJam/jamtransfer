...
<script type="text/javascript">
	...
	var data_v4_SubVehicles = [];
	function all_v4_SubVehicles() {
		...
		var callFunction = 'all_v4_SubVehiclesFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_SubVehicles/v4_SubVehicles_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_SubVehiclesData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_SubVehiclesData.data;
			  recordsTotal = v4_SubVehiclesData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_SubVehicles = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_SubVehiclesListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_SubVehicles : data_v4_SubVehicles});

				$("#show_v4_SubVehicles").html(HTML);
		  },
	...
	function one_v4_SubVehicles(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_SubVehiclesWrapper"+id).css('display') != 'none') {$("#v4_SubVehiclesWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_SubVehicles/v4_SubVehicles_One.php?VehicleID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_SubVehiclesEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_SubVehicles"+id).html(HTML);

				$("#v4_SubVehiclesWrapper"+id).show('slow');
			},
	...
	function new_v4_SubVehicles() { 
		var data = {
				...
				$("#new_v4_SubVehicles").html(HTML);

				$("#v4_SubVehiclesWrapperNew").show('slow');

	}


	function editClosev4_SubVehicles(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_SubVehicles(id, inList) { 
	
		if($("#v4_SubVehiclesEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_SubVehiclesEditForm"+id).serializeObject();
		var formData = $("#v4_SubVehiclesEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_SubVehicles/'+
		'v4_SubVehicles_Save.php?callback=?&keyName=VehicleID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_SubVehiclesData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_SubVehicles(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_SubVehicles/'+
		'v4_SubVehicles_Delete.php?VehicleID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_SubVehicles(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_SubVehiclesData(id, newData) {

	  var result = $.grep(data_v4_SubVehicles, function(e){ return e.VehicleID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_SubVehicles(id, ress);

		    data = data_v4_SubVehicles;

		    var source   = $("#v4_SubVehiclesListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_SubVehicles : data});
	...
	function changeall_v4_SubVehicles( id, sve ) {
	   for (var i in data_v4_SubVehicles) {
		 if (data_v4_SubVehicles[i].VehicleID == id) {
		    data_v4_SubVehicles[i] = sve;
