...
<script type="text/javascript">
	...
	var data_v4_PaymentParams = [];
	function all_v4_PaymentParams() {
		...
		var callFunction = 'all_v4_PaymentParamsFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_PaymentParams/v4_PaymentParams_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_PaymentParamsData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_PaymentParamsData.data;
			  recordsTotal = v4_PaymentParamsData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_PaymentParams = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_PaymentParamsListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_PaymentParams : data_v4_PaymentParams});

				$("#show_v4_PaymentParams").html(HTML);
		  },
	...
	function one_v4_PaymentParams(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_PaymentParamsWrapper"+id).css('display') != 'none') {$("#v4_PaymentParamsWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_PaymentParams/v4_PaymentParams_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_PaymentParamsEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_PaymentParams"+id).html(HTML);

				$("#v4_PaymentParamsWrapper"+id).show('slow');
			},
	...
	function new_v4_PaymentParams() { 
		var data = {
				...
				$("#new_v4_PaymentParams").html(HTML);

				$("#v4_PaymentParamsWrapperNew").show('slow');

	}


	function editClosev4_PaymentParams(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_PaymentParams(id, inList) { 
	
		if($("#v4_PaymentParamsEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_PaymentParamsEditForm"+id).serializeObject();
		var formData = $("#v4_PaymentParamsEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_PaymentParams/'+
		'v4_PaymentParams_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_PaymentParamsData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_PaymentParams(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_PaymentParams/'+
		'v4_PaymentParams_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_PaymentParams(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_PaymentParamsData(id, newData) {

	  var result = $.grep(data_v4_PaymentParams, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_PaymentParams(id, ress);

		    data = data_v4_PaymentParams;

		    var source   = $("#v4_PaymentParamsListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_PaymentParams : data});
	...
	function changeall_v4_PaymentParams( id, sve ) {
	   for (var i in data_v4_PaymentParams) {
		 if (data_v4_PaymentParams[i].ID == id) {
		    data_v4_PaymentParams[i] = sve;
