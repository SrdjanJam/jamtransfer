...
<script type="text/javascript">
	...
	var data_v4_HpImages = [];
	function all_v4_HpImages() {
		...
		var callFunction = 'all_v4_HpImagesFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_HpImages/v4_HpImages_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_HpImagesData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_HpImagesData.data;
			  recordsTotal = v4_HpImagesData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_HpImages = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_HpImagesListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_HpImages : data_v4_HpImages});

				$("#show_v4_HpImages").html(HTML);
		  },
	...
	function one_v4_HpImages(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_HpImagesWrapper"+id).css('display') != 'none') {$("#v4_HpImagesWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_HpImages/v4_HpImages_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_HpImagesEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_HpImages"+id).html(HTML);

				$("#v4_HpImagesWrapper"+id).show('slow');
			},
	...
	function new_v4_HpImages() { 
		var data = {
				...
				$("#new_v4_HpImages").html(HTML);

				$("#v4_HpImagesWrapperNew").show('slow');

	}


	function editClosev4_HpImages(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_HpImages(id, inList) { 
	
		if($("#v4_HpImagesEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_HpImagesEditForm"+id).serializeObject();
		var formData = $("#v4_HpImagesEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_HpImages/'+
		'v4_HpImages_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_HpImagesData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_HpImages(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_HpImages/'+
		'v4_HpImages_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_HpImages(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_HpImagesData(id, newData) {

	  var result = $.grep(data_v4_HpImages, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_HpImages(id, ress);

		    data = data_v4_HpImages;

		    var source   = $("#v4_HpImagesListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_HpImages : data});
	...
	function changeall_v4_HpImages( id, sve ) {
	   for (var i in data_v4_HpImages) {
		 if (data_v4_HpImages[i].ID == id) {
		    data_v4_HpImages[i] = sve;
