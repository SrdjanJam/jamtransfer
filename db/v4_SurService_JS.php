...
<script type="text/javascript">
	...
	var data_v4_SurService = [];
	function all_v4_SurService() {
		...
		var callFunction = 'all_v4_SurServiceFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_SurService/v4_SurService_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_SurServiceData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_SurServiceData.data;
			  recordsTotal = v4_SurServiceData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_SurService = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_SurServiceListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_SurService : data_v4_SurService});

				$("#show_v4_SurService").html(HTML);
		  },
	...
	function one_v4_SurService(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_SurServiceWrapper"+id).css('display') != 'none') {$("#v4_SurServiceWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_SurService/v4_SurService_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_SurServiceEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_SurService"+id).html(HTML);

				$("#v4_SurServiceWrapper"+id).show('slow');
			},
	...
	function new_v4_SurService() { 
		var data = {
				...
				$("#new_v4_SurService").html(HTML);

				$("#v4_SurServiceWrapperNew").show('slow');

	}


	function editClosev4_SurService(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_SurService(id, inList) { 
	
		if($("#v4_SurServiceEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_SurServiceEditForm"+id).serializeObject();
		var formData = $("#v4_SurServiceEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_SurService/'+
		'v4_SurService_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_SurServiceData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_SurService(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_SurService/'+
		'v4_SurService_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_SurService(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_SurServiceData(id, newData) {

	  var result = $.grep(data_v4_SurService, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_SurService(id, ress);

		    data = data_v4_SurService;

		    var source   = $("#v4_SurServiceListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_SurService : data});
	...
	function changeall_v4_SurService( id, sve ) {
	   for (var i in data_v4_SurService) {
		 if (data_v4_SurService[i].ID == id) {
		    data_v4_SurService[i] = sve;
