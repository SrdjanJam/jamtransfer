...
<script type="text/javascript">
	...
	var data_v4_TodoList = [];
	function all_v4_TodoList() {
		...
		var callFunction = 'all_v4_TodoListFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_TodoList/v4_TodoList_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_TodoListData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_TodoListData.data;
			  recordsTotal = v4_TodoListData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_TodoList = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_TodoListListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_TodoList : data_v4_TodoList});

				$("#show_v4_TodoList").html(HTML);
		  },
	...
	function one_v4_TodoList(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_TodoListWrapper"+id).css('display') != 'none') {$("#v4_TodoListWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_TodoList/v4_TodoList_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_TodoListEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_TodoList"+id).html(HTML);

				$("#v4_TodoListWrapper"+id).show('slow');
			},
	...
	function new_v4_TodoList() { 
		var data = {
				...
				$("#new_v4_TodoList").html(HTML);

				$("#v4_TodoListWrapperNew").show('slow');

	}


	function editClosev4_TodoList(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_TodoList(id, inList) { 
	
		if($("#v4_TodoListEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_TodoListEditForm"+id).serializeObject();
		var formData = $("#v4_TodoListEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_TodoList/'+
		'v4_TodoList_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_TodoListData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_TodoList(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_TodoList/'+
		'v4_TodoList_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_TodoList(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_TodoListData(id, newData) {

	  var result = $.grep(data_v4_TodoList, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_TodoList(id, ress);

		    data = data_v4_TodoList;

		    var source   = $("#v4_TodoListListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_TodoList : data});
	...
	function changeall_v4_TodoList( id, sve ) {
	   for (var i in data_v4_TodoList) {
		 if (data_v4_TodoList[i].ID == id) {
		    data_v4_TodoList[i] = sve;
