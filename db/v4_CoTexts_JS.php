...
<script type="text/javascript">
	...
	function all_v4_CoTexts() {
		...
		var callFunction = 'all_v4_CoTextsFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_CoTexts/v4_CoTexts_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_CoTextsData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_CoTextsData.data;
			  recordsTotal = v4_CoTextsData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_CoTexts = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_CoTextsListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_CoTexts : data_v4_CoTexts});

				$("#show_v4_CoTexts").html(HTML);
		  },
	...
	function one_v4_CoTexts(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_CoTextsWrapper"+id).css('display') != 'none') {$("#v4_CoTextsWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_CoTexts/v4_CoTexts_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_CoTextsEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_CoTexts"+id).html(HTML);

				$("#v4_CoTextsWrapper"+id).show('slow');
			},
	...
	function new_v4_CoTexts() { 
		var data = {
				...
				$("#new_v4_CoTexts").html(HTML);

				$("#v4_CoTextsWrapperNew").show('slow');

	}


	function editClosev4_CoTexts(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_CoTexts(id, inList) { 
	
		if($("#v4_CoTextsEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_CoTextsEditForm"+id).serializeObject();
		var formData = $("#v4_CoTextsEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_CoTexts/'+
		'v4_CoTexts_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_CoTextsData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_CoTexts(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_CoTexts/'+
		'v4_CoTexts_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_CoTexts(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_CoTextsData(id, newData) {

	  var result = $.grep(data_v4_CoTexts, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_CoTexts(id, ress);

		    data = data_v4_CoTexts;

		    var source   = $("#v4_CoTextsListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_CoTexts : data});
	...
	function changeall_v4_CoTexts( id, sve ) {
	   for (var i in data_v4_CoTexts) {
		 if (data_v4_CoTexts[i].ID == id) {
		    data_v4_CoTexts[i] = sve;
