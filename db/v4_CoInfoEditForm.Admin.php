...
<script type="text/x-handlebars-template" id="v4_CoInfoEditTemplate">
<form id="v4_CoInfoEditForm{{ID}}" class="form box box-info" enctype="multipart/form-data" method="post" onsubmit="return false;">
		...
		<div class="box-tools pull-right">
			...
			<? if (!$isNew) { ?>
				<? if ($inList=='true') { ?>
					...
					onclick="return editClosev4_CoInfo('{{ID}}', '<?= $inList ?>');">
					<i class="fa fa-arrow-up"></i>
					</button>
				<? } else { ?>
					<button class="btn btn-danger" title="<?= CANCEL ?>" 
					onclick="return deletev4_CoInfo('{{ID}}', '<?= $inList ?>');">
					<i class="fa fa-ban"></i>
					</button>
				<? } ?>	
			<? } ?>	
			<button class="btn btn-info" title="<?= SAVE_CHANGES ?>" 
			onclick="return editSavev4_CoInfo('{{ID}}', '<?= $inList ?>');">
			<i class="ic-disk"></i>
			</button>
			<? if (!$isNew) { ?>
				<button class="btn btn-danger" title="<?= PRINTIT ?>" 
				onclick="return editPrintv4_CoInfo('{{ID}}', '<?= $inList ?>');">
				<i class="ic-print"></i>
				</button>
			<? } ?>	
		</div>
	...
	<div class="box-body ">
        <div class="row">
			<div class="col-md-6">
				...
				<div class="row">
					<div class="col-md-3">
						<label for="co_todo"><?=CO_TODO;?></label>
					</div>
					<div class="col-md-9">
						<textarea name="co_todo" id="co_todo" rows="5" 
					class="textarea" cols="50" style="width:100%">{{co_todo}}</textarea>
					</div>
				</div>
	...
	<div class="box-footer">
		<? if (!$isNew) { ?>
		<div>
    	<button class="btn btn-default" onclick="return deletev4_CoInfo('{{ID}}', '<?= $inList ?>');">
