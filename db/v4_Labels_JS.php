...
<script type="text/javascript">
	...
	function all_v4_Labels() {
		...
		var callFunction = 'all_v4_LabelsFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Labels/v4_Labels_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_LabelsData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_LabelsData.data;
			  recordsTotal = v4_LabelsData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				var source   = $("#v4_LabelsListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Labels : data_v4_Labels});

				$("#show_v4_Labels").html(HTML);
		  },
	...
	function one_v4_Labels(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_LabelsWrapper"+id).css('display') != 'none') {$("#v4_LabelsWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Labels/v4_Labels_One.php?LabelID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_LabelsEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Labels"+id).html(HTML);

				$("#v4_LabelsWrapper"+id).show('slow');
			},
	...
	function new_v4_Labels() { 
		var data = {
				...
				$("#new_v4_Labels").html(HTML);

				$("#v4_LabelsWrapperNew").show('slow');

	}


	function editClosev4_Labels(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Labels(id, inList) { 
	
		if($("#v4_LabelsEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_LabelsEditForm"+id).serializeObject();
		var formData = $("#v4_LabelsEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Labels/'+
		'v4_Labels_Save.php?callback=?&keyName=LabelID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_LabelsData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Labels(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Labels/'+
		'v4_Labels_Delete.php?LabelID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Labels(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_LabelsData(id, newData) {

	  var result = $.grep(data_v4_Labels, function(e){ return e.LabelID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    var HTML = template({v4_Labels : data});
	...
	function changeall_v4_Labels( id, sve ) {
	   for (var i in data_v4_Labels) {
		 if (data_v4_Labels[i].LabelID == id) {
		    data_v4_Labels[i] = sve;
