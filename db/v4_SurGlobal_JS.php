...
<script type="text/javascript">
	...
	var data_v4_SurGlobal = [];
	function all_v4_SurGlobal() {
		...
		var callFunction = 'all_v4_SurGlobalFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_SurGlobal/v4_SurGlobal_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_SurGlobalData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_SurGlobalData.data;
			  recordsTotal = v4_SurGlobalData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_SurGlobal = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_SurGlobalListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_SurGlobal : data_v4_SurGlobal});

				$("#show_v4_SurGlobal").html(HTML);
		  },
	...
	function one_v4_SurGlobal(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_SurGlobalWrapper"+id).css('display') != 'none') {$("#v4_SurGlobalWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_SurGlobal/v4_SurGlobal_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_SurGlobalEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_SurGlobal"+id).html(HTML);

				$("#v4_SurGlobalWrapper"+id).show('slow');
			},
	...
	function new_v4_SurGlobal() { 
		var data = {
				...
				$("#new_v4_SurGlobal").html(HTML);

				$("#v4_SurGlobalWrapperNew").show('slow');

	}


	function editClosev4_SurGlobal(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_SurGlobal(id, inList) { 
	
		if($("#v4_SurGlobalEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_SurGlobalEditForm"+id).serializeObject();
		var formData = $("#v4_SurGlobalEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_SurGlobal/'+
		'v4_SurGlobal_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_SurGlobalData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_SurGlobal(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_SurGlobal/'+
		'v4_SurGlobal_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_SurGlobal(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_SurGlobalData(id, newData) {

	  var result = $.grep(data_v4_SurGlobal, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_SurGlobal(id, ress);

		    data = data_v4_SurGlobal;

		    var source   = $("#v4_SurGlobalListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_SurGlobal : data});
	...
	function changeall_v4_SurGlobal( id, sve ) {
	   for (var i in data_v4_SurGlobal) {
		 if (data_v4_SurGlobal[i].ID == id) {
		    data_v4_SurGlobal[i] = sve;
