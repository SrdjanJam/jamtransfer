...
<script type="text/javascript">
	...
	var data_v4_Policies_DE = [];
	function all_v4_Policies_DE() {
		...
		var callFunction = 'all_v4_Policies_DEFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Policies_DE/v4_Policies_DE_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_Policies_DEData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_Policies_DEData.data;
			  recordsTotal = v4_Policies_DEData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Policies_DE = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_Policies_DEListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Policies_DE : data_v4_Policies_DE});

				$("#show_v4_Policies_DE").html(HTML);
		  },
	...
	function one_v4_Policies_DE(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_Policies_DEWrapper"+id).css('display') != 'none') {$("#v4_Policies_DEWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Policies_DE/v4_Policies_DE_One.php?OwnerID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_Policies_DEEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Policies_DE"+id).html(HTML);

				$("#v4_Policies_DEWrapper"+id).show('slow');
			},
	...
	function new_v4_Policies_DE() { 
		var data = {
				...
				$("#new_v4_Policies_DE").html(HTML);

				$("#v4_Policies_DEWrapperNew").show('slow');

	}


	function editClosev4_Policies_DE(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Policies_DE(id, inList) { 
	
		if($("#v4_Policies_DEEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_Policies_DEEditForm"+id).serializeObject();
		var formData = $("#v4_Policies_DEEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Policies_DE/'+
		'v4_Policies_DE_Save.php?callback=?&keyName=OwnerID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_Policies_DEData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Policies_DE(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Policies_DE/'+
		'v4_Policies_DE_Delete.php?OwnerID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Policies_DE(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_Policies_DEData(id, newData) {

	  var result = $.grep(data_v4_Policies_DE, function(e){ return e.OwnerID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Policies_DE(id, ress);

		    data = data_v4_Policies_DE;

		    var source   = $("#v4_Policies_DEListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Policies_DE : data});
	...
	function changeall_v4_Policies_DE( id, sve ) {
	   for (var i in data_v4_Policies_DE) {
		 if (data_v4_Policies_DE[i].OwnerID == id) {
		    data_v4_Policies_DE[i] = sve;
