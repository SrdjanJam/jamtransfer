...
<script type="text/javascript">
	...
	var data_v4_ServicesCopy = [];
	function all_v4_ServicesCopy() {
		...
		var callFunction = 'all_v4_ServicesCopyFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_ServicesCopy/v4_ServicesCopy_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_ServicesCopyData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_ServicesCopyData.data;
			  recordsTotal = v4_ServicesCopyData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_ServicesCopy = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_ServicesCopyListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_ServicesCopy : data_v4_ServicesCopy});

				$("#show_v4_ServicesCopy").html(HTML);
		  },
	...
	function one_v4_ServicesCopy(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_ServicesCopyWrapper"+id).css('display') != 'none') {$("#v4_ServicesCopyWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_ServicesCopy/v4_ServicesCopy_One.php?ServiceID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_ServicesCopyEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_ServicesCopy"+id).html(HTML);

				$("#v4_ServicesCopyWrapper"+id).show('slow');
			},
	...
	function new_v4_ServicesCopy() { 
		var data = {
				...
				$("#new_v4_ServicesCopy").html(HTML);

				$("#v4_ServicesCopyWrapperNew").show('slow');

	}


	function editClosev4_ServicesCopy(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_ServicesCopy(id, inList) { 
	
		if($("#v4_ServicesCopyEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_ServicesCopyEditForm"+id).serializeObject();
		var formData = $("#v4_ServicesCopyEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_ServicesCopy/'+
		'v4_ServicesCopy_Save.php?callback=?&keyName=ServiceID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_ServicesCopyData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_ServicesCopy(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_ServicesCopy/'+
		'v4_ServicesCopy_Delete.php?ServiceID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_ServicesCopy(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_ServicesCopyData(id, newData) {

	  var result = $.grep(data_v4_ServicesCopy, function(e){ return e.ServiceID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_ServicesCopy(id, ress);

		    data = data_v4_ServicesCopy;

		    var source   = $("#v4_ServicesCopyListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_ServicesCopy : data});
	...
	function changeall_v4_ServicesCopy( id, sve ) {
	   for (var i in data_v4_ServicesCopy) {
		 if (data_v4_ServicesCopy[i].ServiceID == id) {
		    data_v4_ServicesCopy[i] = sve;
