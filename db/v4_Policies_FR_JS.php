...
<script type="text/javascript">
	...
	var data_v4_Policies_FR = [];
	function all_v4_Policies_FR() {
		...
		var callFunction = 'all_v4_Policies_FRFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_Policies_FR/v4_Policies_FR_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_Policies_FRData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_Policies_FRData.data;
			  recordsTotal = v4_Policies_FRData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Policies_FR = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_Policies_FRListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Policies_FR : data_v4_Policies_FR});

				$("#show_v4_Policies_FR").html(HTML);
		  },
	...
	function one_v4_Policies_FR(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_Policies_FRWrapper"+id).css('display') != 'none') {$("#v4_Policies_FRWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_Policies_FR/v4_Policies_FR_One.php?OwnerID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_Policies_FREditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Policies_FR"+id).html(HTML);

				$("#v4_Policies_FRWrapper"+id).show('slow');
			},
	...
	function new_v4_Policies_FR() { 
		var data = {
				...
				$("#new_v4_Policies_FR").html(HTML);

				$("#v4_Policies_FRWrapperNew").show('slow');

	}


	function editClosev4_Policies_FR(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Policies_FR(id, inList) { 
	
		if($("#v4_Policies_FREditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_Policies_FREditForm"+id).serializeObject();
		var formData = $("#v4_Policies_FREditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Policies_FR/'+
		'v4_Policies_FR_Save.php?callback=?&keyName=OwnerID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_Policies_FRData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Policies_FR(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_Policies_FR/'+
		'v4_Policies_FR_Delete.php?OwnerID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Policies_FR(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_Policies_FRData(id, newData) {

	  var result = $.grep(data_v4_Policies_FR, function(e){ return e.OwnerID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Policies_FR(id, ress);

		    data = data_v4_Policies_FR;

		    var source   = $("#v4_Policies_FRListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Policies_FR : data});
	...
	function changeall_v4_Policies_FR( id, sve ) {
	   for (var i in data_v4_Policies_FR) {
		 if (data_v4_Policies_FR[i].OwnerID == id) {
		    data_v4_Policies_FR[i] = sve;
