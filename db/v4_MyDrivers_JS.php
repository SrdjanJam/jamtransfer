...
<script type="text/javascript">
	...
	var data_v4_MyDrivers = [];
	function all_v4_MyDrivers() {
		...
		var callFunction = 'all_v4_MyDriversFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = WEBSITEURL + '/cms/p/modules/v4_MyDrivers/v4_MyDrivers_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_MyDriversData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_MyDriversData.data;
			  recordsTotal = v4_MyDriversData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_MyDrivers = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_MyDriversListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_MyDrivers : data_v4_MyDrivers});

				$("#show_v4_MyDrivers").html(HTML);
		  },
	...
	function one_v4_MyDrivers(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_MyDriversWrapper"+id).css('display') != 'none') {$("#v4_MyDriversWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = WEBSITEURL + '/cms/p/modules/v4_MyDrivers/v4_MyDrivers_One.php?DriverID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_MyDriversEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_MyDrivers"+id).html(HTML);

				$("#v4_MyDriversWrapper"+id).show('slow');
			},
	...
	function new_v4_MyDrivers() { 
		var data = {
				...
				$("#new_v4_MyDrivers").html(HTML);

				$("#v4_MyDriversWrapperNew").show('slow');

	}


	function editClosev4_MyDrivers(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_MyDrivers(id, inList) { 
	
		if($("#v4_MyDriversEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_MyDriversEditForm"+id).serializeObject();
		var formData = $("#v4_MyDriversEditForm"+id).serialize();

		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_MyDrivers/'+
		'v4_MyDrivers_Save.php?callback=?&keyName=DriverID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_MyDriversData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_MyDrivers(id, inList) { 
		...
		// update data on server
		var url = WEBSITEURL + '/cms/p/modules/v4_MyDrivers/'+
		'v4_MyDrivers_Delete.php?DriverID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_MyDrivers(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_MyDriversData(id, newData) {

	  var result = $.grep(data_v4_MyDrivers, function(e){ return e.DriverID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_MyDrivers(id, ress);

		    data = data_v4_MyDrivers;

		    var source   = $("#v4_MyDriversListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_MyDrivers : data});
	...
	function changeall_v4_MyDrivers( id, sve ) {
	   for (var i in data_v4_MyDrivers) {
		 if (data_v4_MyDrivers[i].DriverID == id) {
		    data_v4_MyDrivers[i] = sve;
