...
$query="SELECT * FROM `v4_OrdersMasterTemp` WHERE `MPaymentStatus`=1 and `MOrderDate`=CURDATE()
			and `MOrderTime`>'".$timeFrom."' and `MOrderTime`< '".$timeTo."' and `MPaymentMethod` in (1,3) and `MOrderKey` not in (SELECT `MCardNumber` FROM `v4_OrdersMaster` WHERE `MOrderDate`=CURDATE()
			and `MOrderTime`>'".$timeFrom."' and `MOrderTime`< '".$timeTo."'  and `MPaymentMethod` in (1,3)) and `MPayNow`>0";
$od = $db->RunQuery($query);
while($odk = $od->fetch_object()) {
	$au->getRow($odk->UserID);
	$message="Unfinished payment: Pax ". $odk->MPaxFirstName." ".$odk->MPaxLastName;
	// slanje notifikacije
	$nt->setMessage($message);
	//$nt->setCNotificationID();
	//$nt->setSenderID();
