...
<script type="text/x-handlebars-template" id="v4_ExtrasMasterEditTemplate">
<form id="v4_ExtrasMasterEditForm{{ID}}" class="form box box-info" enctype="multipart/form-data" method="post" onsubmit="return false;">
	<div class="box-header">
		<div class="box-title">
			<? if ($isNew) { ?>
				<h3><?= NNEW ?></h3>
			<? } else { ?>
				<h3><?= EDIT ?> - {{ServiceEN}}</h3>
			<? } ?>
		</div>
		<div class="box-tools pull-right">
			...
			<? if (!$isNew) { ?>
				<? if ($inList=='true') { ?>
					...
					onclick="return editClosev4_ExtrasMaster('{{ID}}', '<?= $inList ?>');">
					<i class="fa fa-arrow-up"></i>
					</button>
				<? } else { ?>
					<button class="btn btn-danger" title="<?= CANCEL ?>" 
					onclick="return deletev4_ExtrasMaster('{{ID}}', '<?= $inList ?>');">
					<i class="fa fa-ban"></i>
					</button>
				<? } ?>	
			<? } ?>	
			<button class="btn btn-info" title="<?= SAVE_CHANGES ?>" 
			onclick="return editSavev4_ExtrasMaster('{{ID}}', '<?= $inList ?>');">
			<i class="ic-disk"></i>
			</button>
			<!--
				<? if (!$isNew) { ?>
					<button class="btn btn-danger" title="<?= PRINTIT ?>" 
					onclick="return editPrintv4_ExtrasMaster('{{ID}}', '<?= $inList ?>');">
					<i class="ic-print"></i>
					</button>
				<? } ?>	
			-->
	...
	<div class="box-footer">
		<? /*if (!$isNew) { ?>
		<div>
    	<button class="btn btn-default" onclick="return deletev4_ExtrasMaster('{{ID}}', '<?= $inList ?>');">
