...
<script type="text/javascript">
	...
	function all_v4_Pages() {
		...
		var callFunction = 'all_v4_PagesFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = window.root + '/cms/p/modules/v4_Pages/v4_Pages_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_PagesData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_PagesData.data;
			  recordsTotal = v4_PagesData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				var source   = $("#v4_PagesListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Pages : data_v4_Pages});

				$("#show_v4_Pages").html(HTML);
		  },
	...
	function one_v4_Pages(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_PagesWrapper"+id).css('display') != 'none') {$("#v4_PagesWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = window.root + '/cms/p/modules/v4_Pages/v4_Pages_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_PagesEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Pages"+id).html(HTML);

				$("#v4_PagesWrapper"+id).show('slow');
			},
	...
	function new_v4_Pages() { 
		var data = {
				...
				$("#new_v4_Pages").html(HTML);

				$("#v4_PagesWrapperNew").show('slow');

	}


	function editClosev4_Pages(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow'); }
		return false;
	}

	function editSavev4_Pages(id, inList) { 
	
		if($("#v4_PagesEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_PagesEditForm"+id).serializeObject();
		var formData = $("#v4_PagesEditForm"+id).serialize();

		
		<?/*
		// update data on server
		//var url = window.root + '/cms/p/modules/v4_Pages/'+
		//'v4_Pages_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		
		stara shema nije radila jer je POST bio predug za Terms&Cond.
		Umjesto toga stavio sam ovo dolje.
		callback MORA ostati u url!
		data se moze sloziti kao sto je dolje.
		$.extend spaja dva objekta.
		*/?>
		var url = window.root + '/cms/p/modules/v4_Pages/v4_Pages_Save.php?callback=?';
		
		var data = $.extend({"keyName":"ID", "keyValue":id}, newData);
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_PagesData(id, newData);
					$(".editFrame").hide('slow');
				}
			},
	...
	function deletev4_Pages(id, inList) { 
		...
		// update data on server
		var url = window.root + '/cms/p/modules/v4_Pages/'+
		'v4_Pages_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Pages(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_PagesData(id, newData) {

	  var result = $.grep(data_v4_Pages, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    var HTML = template({v4_Pages : data});
	...
	function changeall_v4_Pages( id, sve ) {
	   for (var i in data_v4_Pages) {
		 if (data_v4_Pages[i].ID == id) {
		    data_v4_Pages[i] = sve;
