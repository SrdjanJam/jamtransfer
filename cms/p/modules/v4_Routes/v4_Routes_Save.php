...
$noRoutes = 0;
if(isset($_REQUEST['FromID']) && isset($_REQUEST['ToID'])) {
	$PlaceFromID=$_REQUEST['FromID'];
	$PlaceToID=$_REQUEST['ToID'];
	$rKeys = $db->getKeysBy('RouteID', 'ASC', 'WHERE ((FromID = ' . $PlaceFromID . ' AND ToID = ' . $PlaceToID . ') OR (ToID = ' . $PlaceFromID . ' AND FromID = ' . $PlaceToID . '))');
	$noRoutes = count($rKeys); 		
}
...
if ($keyName != '' and $keyValue != '' or ($noRoutes>0)) {
	$res = $db->saveRow();
	$upd = 'Updated';
	if($res !== true) $upd = $res;  
}
