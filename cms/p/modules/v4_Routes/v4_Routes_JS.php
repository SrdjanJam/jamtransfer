...
<script type="text/javascript">
	...
	function all_v4_Routes() {
		...
		var callFunction = 'all_v4_RoutesFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = window.root + '/cms/p/modules/v4_Routes/v4_Routes_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_RoutesData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_RoutesData.data;
			  recordsTotal = v4_RoutesData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				var source   = $("#v4_RoutesListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Routes : data_v4_Routes});

				$("#show_v4_Routes").html(HTML);
		  },
	...
	function one_v4_Routes(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_RoutesWrapper"+id).css('display') != 'none') {$("#v4_RoutesWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = window.root + '/cms/p/modules/v4_Routes/v4_Routes_One.php?RouteID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_RoutesEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Routes"+id).html(HTML);

				$("#v4_RoutesWrapper"+id).show('slow');
			},
	...
	function new_v4_Routes() { 
		var data = {
				...
				$("#new_v4_Routes").html(HTML);

				$("#v4_RoutesWrapperNew").show('slow');

	}


	function editClosev4_Routes(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Routes(id, inList) { 
	
		if($("#v4_RoutesEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_RoutesEditForm"+id).serializeObject();
		var formData = $("#v4_RoutesEditForm"+id).serialize();

		// update data on server
		var url = window.root + '/cms/p/modules/v4_Routes/'+
		'v4_Routes_Save.php?callback=?&keyName=RouteID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_RoutesData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Routes(id, inList) { 
		...
		// update data on server
		var url = window.root + '/cms/p/modules/v4_Routes/'+
		'v4_Routes_Delete.php?RouteID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Routes(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_RoutesData(id, newData) {

	  var result = $.grep(data_v4_Routes, function(e){ return e.RouteID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    var HTML = template({v4_Routes : data});
	...
	function changeall_v4_Routes( id, sve ) {
	   for (var i in data_v4_Routes) {
		 if (data_v4_Routes[i].RouteID == id) {
		    data_v4_Routes[i] = sve;
