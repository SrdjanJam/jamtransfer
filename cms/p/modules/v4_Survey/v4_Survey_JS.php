...
<script type="text/javascript">
	...
	function all_v4_Survey() {
		...
		var callFunction = 'all_v4_SurveyFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = window.root + '/cms/p/modules/v4_Survey/v4_Survey_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_SurveyData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_SurveyData.data;
			  recordsTotal = v4_SurveyData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				var source   = $("#v4_SurveyListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Survey : data_v4_Survey});

				$("#show_v4_Survey").html(HTML);
		  },
	...
	function one_v4_Survey(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_SurveyWrapper"+id).css('display') != 'none') {$("#v4_SurveyWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = window.root + '/cms/p/modules/v4_Survey/v4_Survey_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_SurveyEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Survey"+id).html(HTML);

				$("#v4_SurveyWrapper"+id).show('slow');
			},
	...
	function new_v4_Survey() { 
		var data = {
				...
				$("#new_v4_Survey").html(HTML);

				$("#v4_SurveyWrapperNew").show('slow');

	}


	function editClosev4_Survey(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Survey(id, inList) { 
	
		if($("#v4_SurveyEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_SurveyEditForm"+id).serializeObject();
		var formData = $("#v4_SurveyEditForm"+id).serialize();

		// update data on server
		var url = window.root + '/cms/p/modules/v4_Survey/'+
		'v4_Survey_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_SurveyData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Survey(id, inList) { 
		...
		// update data on server
		var url = window.root + '/cms/p/modules/v4_Survey/'+
		'v4_Survey_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Survey(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_SurveyData(id, newData) {

	  var result = $.grep(data_v4_Survey, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    var HTML = template({v4_Survey : data});
	...
	function changeall_v4_Survey( id, sve ) {
	   for (var i in data_v4_Survey) {
		 if (data_v4_Survey[i].ID == id) {
		    data_v4_Survey[i] = sve;
