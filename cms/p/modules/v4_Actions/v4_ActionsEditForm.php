...
<script type="text/x-handlebars-template" id="v4_ActionsEditTemplate">
<form id="v4_ActionsEditForm{{ID}}" class="form box box-info" enctype="multipart/form-data" method="post" onsubmit="return false;">
		...
		<div class="box-tools pull-right">
			...
			<? if (!$isNew) { ?>
				<? if ($inList=='true') { ?>
					...
					onclick="return editClosev4_Actions('{{ID}}', '<?= $inList ?>');">
					<i class="fa fa-arrow-up"></i>
					</button>
				<? } else { ?>
					<button class="btn btn-danger" title="<?= CANCEL ?>" 
					onclick="return deletev4_Actions('{{ID}}', '<?= $inList ?>');">
					<i class="fa fa-ban"></i>
					</button>
				<? } ?>	
			<? } ?>	
			<button class="btn btn-info" title="<?= SAVE_CHANGES ?>" 
			onclick="return editSavev4_Actions('{{ID}}', '<?= $inList ?>');">
			<i class="ic-disk"></i>
			</button>
			<!--
				<? if (!$isNew) { ?>
					<button class="btn btn-danger" title="<?= PRINTIT ?>" 
					onclick="return editPrintv4_Actions('{{ID}}', '<?= $inList ?>');">
					<i class="ic-print"></i>
					</button>
				<? } ?>	
			-->
