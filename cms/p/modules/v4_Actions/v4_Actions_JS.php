...
<script type="text/javascript">
	...
	function all_v4_Actions() {
		...
		var callFunction = 'all_v4_ActionsFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = window.root + '/cms/p/modules/v4_Actions/v4_Actions_All.php?where='+where+
...
console.log (url);
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_ActionsData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_ActionsData.data;
			  recordsTotal = v4_ActionsData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_Actions = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_ActionsListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_Actions : data_v4_Actions});

				$("#show_v4_Actions").html(HTML);
		  },
	...
	function one_v4_Actions(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_ActionsWrapper"+id).css('display') != 'none') {$("#v4_ActionsWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = window.root + '/cms/p/modules/v4_Actions/v4_Actions_One.php?ID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_ActionsEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_Actions"+id).html(HTML);

				$("#v4_ActionsWrapper"+id).show('slow');
			},
	...
	function new_v4_Actions() { 
		var data = {
				...
				$("#new_v4_Actions").html(HTML);

				$("#v4_ActionsWrapperNew").show('slow');

	}


	function editClosev4_Actions(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow');$(".editFrame form").html(''); }
		return false;
	}

	function editSavev4_Actions(id, inList) {  
		if($("#v4_ActionsEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_ActionsEditForm"+id).serializeObject();
		var formData = $("#v4_ActionsEditForm"+id).serialize();
console.log( window.root + '/cms/p/modules/v4_Actions/'+
		'v4_Actions_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData)
		// update data on server
		var url = window.root + '/cms/p/modules/v4_Actions/'+
		'v4_Actions_Save.php?callback=?&keyName=ID&keyValue='+id+'&'+ formData;
		$.ajax({
			type: 'POST',
			url: url,
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_ActionsData(id, newData);
					$(".editFrame").hide('slow');
					$(".editFrame form").html('');
				}
			},
	...
	function deletev4_Actions(id, inList) { 
		...
		// update data on server
		var url = window.root + '/cms/p/modules/v4_Actions/'+
		'v4_Actions_Delete.php?ID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_Actions(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_ActionsData(id, newData) {

	  var result = $.grep(data_v4_Actions, function(e){ return e.ID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_Actions(id, ress);

		    data = data_v4_Actions;

		    var source   = $("#v4_ActionsListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_Actions : data});
	...
	function changeall_v4_Actions( id, sve ) {
	   for (var i in data_v4_Actions) {
		 if (data_v4_Actions[i].ID == id) {
		    data_v4_Actions[i] = sve;
