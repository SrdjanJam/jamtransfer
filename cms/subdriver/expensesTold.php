<?
	...
	if (isset($_FILES["DocumentImage"]) && !empty(basename($_FILES["DocumentImage"]["name"]))) { 
		...
		$target_doc_file = $target_doc_dir . basename($_FILES["DocumentImage"]["name"]);
		$uploadOk = 1;
	...
	if (isset($_FILES["ActionImage"]) && !empty(basename($_FILES["ActionImage"]["name"]))) { 
		...
		$target_act_file = $target_act_dir . basename($_FILES["ActionImage"]["name"]);
		$uploadOk = 1;
		$imageFileType = strtolower(pathinfo($target_act_file,PATHINFO_EXTENSION));
		// Check if image file is a actual image or fake image
		if(isset($_POST["addExpense"])) {
			$check = getimagesize($_FILES["ActionImage"]["tmp_name"]);
			if($check !== false) move_uploaded_file($_FILES["ActionImage"]["tmp_name"],$target_act_file);
			else echo "File is not an image.";			
		}
		...
		)
	{
			...
			)
		{
			...
			$q 	= "INSERT INTO v4_SubExpenses (OwnerID, DriverID, Datum, Expense, Amount, Card, DocumentImage, ActionImage, ApprovedFuelPrice) ";
			$q .= "VALUES('";
			$q .= $_REQUEST['OwnerID'] ."','";
			$q .= $_REQUEST['DriverID'] ."','";
			$q .= $_REQUEST['Datum'] ."','";
			...
			$q .= $_REQUEST['Amount'] ."','";
			$q .= $_REQUEST['Card'] ."','";
			$q .= "https://cms.jamtransfer.com/cms/".$target_doc_dir."','";
			$q .= $target_act_file ."','";
			$q .= $afp ."')";
...
<div class="container white">
	...
	<div class="row">
		<div class="col-xs-12 pad1em">
		    <h3>DAILY ACTIVITIES <?= date("d.m.Y") ?></h3> 
		    <br><br>
		    <?
...
   <form  action="index.php?p=expenses" method="POST" class="pad1em no-print" enctype="multipart/form-data">
		<div class="row">
			<div class="col-sm-6">
		        <label for="Datum">Action date</label>
			</div>
			<div class="col-sm-6">
		...
		<div class="row">
			<div class="col-sm-6">
		        <label for="Expense">Action type</label>
			</div>
			<div class="col-sm-6">
		...
		        <label for="ActionImage">Action Image</label>
			...
			<div class="col-sm-6">
				<input id='photo_ac' type="file" style='display:none' name="ActionImage" accept="image/*" capture="camera">
				<button id='photo2_ac' type="button" name="photo_ac"  class="btn btn-primary btn-block ">TAKE A PHOTO</button>		
			</div>
		</div>		
	...
	<form id="expensesList" class="pad1em" action="index.php?p=expenses" method="POST">
		<? foreach($ex as $key => $exp) { ?>
			<div class="row table" style="margin:6px 0">
				...
				<div class="col-sm-2" style="text-align:right">		
					<img  class="small" src="<?= $exp['ActionImage'] ?>" alt="" height="50" width="50">
				</div>				
				<div class="col-sm-2" style="text-align:right">
