<?
	...
	if (isset($_FILES["DocumentImage"]) && !empty(basename($_FILES["DocumentImage"]["name"]))) { 
		...
		if(isset($_POST["addExpense"])) {
			...
			if($check !== false) move_uploaded_file($_FILES["DocumentImage"]["tmp_name"],$target_file);
		...
		)
	{
			...
			)
		{
			...
			$q 	= "INSERT INTO v4_SubExpenses (OwnerID, DriverID, Datum, Expense, Amount, Card, DocumentImage, ApprovedFuelPrice) ";
			$q .= "VALUES('";
			$q .= $_REQUEST['OwnerID'] ."','";
			$q .= $_REQUEST['DriverID'] ."','";
			$q .= $_REQUEST['Datum'] ."','";
			...
			$q .= $_REQUEST['Amount'] ."','";
			$q .= $_REQUEST['Card'] ."','";
			$q .= "https://cms.jamtransfer.com/cms/".$target_file ."','";
			$q .= $afp ."')";
