<?
	...
	if( isset($_REQUEST['Save']) and 
		...
		if($u['CountryName'] != 'Serbia') {
			...
			require_once ROOT . '/plugins/AgentsTransfers/invoiceForeign.php';
		}
		else {
			// echo('test');
			require_once ROOT . '/plugins/AgentsTransfers/invoiceSerbian.php';
		}
		...
		if($knjigovodstvo == '1') {
			...
			$replaceChars = array('/','\\');
			$PDFfile = 	'pdf/RA_'. str_replace($replaceChars,'-',$_REQUEST['InvoiceNumber']).'.pdf';
