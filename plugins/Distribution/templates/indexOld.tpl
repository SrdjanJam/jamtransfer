...
<div class="container-fluid pad1em">
    <p class="lead">{$smarty.request.DateFrom} - {$smarty.request.DateTo}</p>

	<form action="index.php?p=timetable" method="post" style="display:inline-block;width:30%;">
		<input name='DateFrom' id='DateFrom' type="hidden" value='{$smarty.request.DateFrom}' />
		<input name='DateTo' id='DateTo' type="hidden" value='{$smarty.request.DateTo}' />
		<input name='SubDriverID' id='SubDriverID' type="hidden" value='{$smarty.request.SubDriverID}' />
		<input name='SortSubDriver' id='SortSubDriver' type="hidden" value='{$smarty.request.SortSubDriver}' />
		<input name='OwnerID' id='OwnerID' type="hidden" value='{$smarty.session.OwnerID}' />
		{$SORT}:
		<div class="btn-group">
...
<?	
	...
	foreach($odArray as $val => $ID) {
		...
		$totalValue += $od->getDetailPrice()+$od->getExtraCharge();
	...
	?>    
		...
		style="cursor:default; padding:8px !important;background:{$$bgColor}">
		    <div class="col-md-2">
				...
				<div class="small">
				    <? if(!$fakeDriverFound) {}
					    <a href="https://cms.jamtransfer.com/cms/printTransfer.php?OrderID={$$od->getOrderID()}" target="_blank">{$$om->getMOrderKey();?>-
					    {$$od->getOrderID();}-{$$od->getTNo()}</a><br>
						Booked: {$$od->getOrderDate()?>
					<? } else {}
					    <a href="https://cms.jamtransfer.com/cms/printTransferFR.php?DetailsID=
		    ...
		    <div class="col-md-3">
				<div class="row">
					<div class="col-md-6">
					    <?//= $changedIcon}
						<input type="text" class="timepicker w50 {$$color}" id="SubPickupTime_{$$i}"   
