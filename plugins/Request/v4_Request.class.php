...
Class v4_Request {
     ...
     *
     */
	public function New_v4_Request($DisplayOrder,$Title,$ServiceDE,$ServiceRU,$ServiceFR,$ServiceIT,$ServiceSE,$ServiceNO){
	...
	public function getRow($key_row){
		$result = $this->connection->RunQuery("Select * from v4_Request where ID = \"$key_row\" ");
	...
	public function deleteRow($key_row){
		$this->connection->RunQuery("DELETE FROM v4_Request WHERE ID = $key_row");
	}
	...
	public function saveRow(){
		$result = $this->connection->RunQuery("UPDATE v4_Request set 
...
ServiceNO = '".$this->myreal_escape_string($this->ServiceNO)."' WHERE ID = '".$this->ID."'");
	...
	public function getKeysBy($column, $order, $where = NULL){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT ID from v4_Request $where ORDER BY $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
				$keys[$i] = $row["ID"];
				$i++;
			}
