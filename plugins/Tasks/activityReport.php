...
if( isset($_REQUEST['Save']) and $_REQUEST['Save'] == '1') {
    ...
    foreach($_REQUEST['Approved'] as $ID => $Approved) {
        //echo $ID . '=>' . $Approved . '<br>';
        $qu = "UPDATE v4_SubActivitiy SET Approved = '" . $Approved . "' WHERE ID = '" . $ID . "'";
        $db->RunQuery($qu);
    }
}
...
$q .= ' WHERE v4_SubActivitiy.Approved <9 AND v4_SubActivitiy.OwnerID = '.$SOwnerID;

if ($dateFrom != null) $q .= " AND Datum >= '".$dateFrom."'";
if ($dateTo != null) $q .= " AND Datum <= '".$dateTo."'";
if ($DriverID != "0") $q .= ' AND v4_SubActivitiy.DriverID = '.$DriverID;
...
<div class="container">
	...
	<form action="p/modules/v4_SubActivitiy/expensesReportPrint.php" method="post" target="_blank" style="float:right;margin-top:-30px">
