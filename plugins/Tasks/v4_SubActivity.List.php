...
$q = "SELECT AuthUserID, AuthUserRealName FROM v4_AuthUsers WHERE Active=1 AND AuthLevelID = 32 AND DriverID = ".$SOwnerID." ORDER BY AuthUserRealName ASC";
$r = $db->RunQuery($q);
$driverArr = array();
while($e = $r->fetch_object()) {
	$driverArr[] = $e;
...
$q = "SELECT VehicleID, VehicleDescription FROM v4_SubVehicles WHERE Active=1 AND OwnerID = ".$SOwnerID." ORDER BY VehicleDescription ASC";
$r = $db->RunQuery($q);
$vehicleArr = array();
while($e = $r->fetch_object()) {
	$vehicleArr[$e->VehicleID] = $e;
}
...
?>
<div class="container">
	<h1><?= TASK ?> <?= $titleAddOn ?></h1>
	<a class="btn btn-primary btn-xs" href="index.php?p=new_v4_SubTasks"><?= NNEW ?></a>
	<br><br>
