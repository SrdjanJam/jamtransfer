...
$q = "SELECT AuthUserID, AuthUserRealName FROM v4_AuthUsers WHERE Active=1 AND AuthLevelID = '32' AND DriverID = '".$_SESSION["OwnerID"]."' ORDER BY AuthUserRealName ASC";
$r = $db->RunQuery($q);
$driverArr = array();
...
$q = "SELECT VehicleID, VehicleDescription FROM v4_SubVehicles WHERE Active=1 AND OwnerID = '".$_SESSION["OwnerID"]."' ORDER BY VehicleDescription ASC";
$r = $db->RunQuery($q);
$vehicleArr = array();
while($e = $r->fetch_object()) {
	$vehicleArr[] = $e;
...
<div class="container">
	...
	<form action="index.php?p=subtasks" method="POST">
		<h1><?= TASKS ?></h1>
		...
		<input class="btn btn-primary" type="submit" value="Show tasks" name="submit">
	</form>
