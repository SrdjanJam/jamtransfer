...
<script type="text/javascript">
	...
	var data_v4_PlaceTypes = [];
	function all_v4_PlaceTypes() {
		...
		var callFunction = 'all_v4_PlaceTypesFilter()'; // funkcija koju paginator poziva kod promjene stranice
		...
		//
	 	...
	 	var url = window.root + '/cms/p/modules/v4_PlaceTypes/v4_PlaceTypes_All.php?where='+where+
		...
		$.ajax({
		 type: 'GET',
		  ...
		  dataType: 'jsonp',
		  success: function(v4_PlaceTypesData) {

			  // CUSTOM STUFF
			  // uzmi samo podatke o transferima
			  var data = v4_PlaceTypesData.data;
			  recordsTotal = v4_PlaceTypesData.recordsTotal;
			  ...
			  $.each(data, function(i, item) {
				...
				data_v4_PlaceTypes = data;
			
				// poziva se handlebars da pripremi prikaz
				// template je u parts/transferList.Driver.php

				var source   = $("#v4_PlaceTypesListTemplate").html();
				var template = Handlebars.compile(source);

				var HTML = template({v4_PlaceTypes : data_v4_PlaceTypes});

				$("#show_v4_PlaceTypes").html(HTML);
		  },
	...
	function one_v4_PlaceTypes(id,inList) { 
		...
		if(inList==true) {
			if ( $("#v4_PlaceTypesWrapper"+id).css('display') != 'none') {$("#v4_PlaceTypesWrapper"+id).hide('slow'); return;}
		}

		// ako element nije prikazan, uzmi potrebne podatke i prikazi ga
		var url = window.root + '/cms/p/modules/v4_PlaceTypes/v4_PlaceTypes_One.php?PlaceTypeID='+id;
		...
		$.ajax({
			...
			success: function(data) {
				...
				var source   = $("#v4_PlaceTypesEditTemplate").html();
				var template = Handlebars.compile(source);
				...
				$("#one_v4_PlaceTypes"+id).html(HTML);

				$("#v4_PlaceTypesWrapper"+id).show('slow');
			},
	...
	function new_v4_PlaceTypes() { 
		var data = {
				...
				$("#new_v4_PlaceTypes").html(HTML);

				$("#v4_PlaceTypesWrapperNew").show('slow');

	}


	function editClosev4_PlaceTypes(id, inList) {
		if (notDefined(inList)) {inList=true;}
		if (inList==true) { $(".editFrame").hide('slow'); }
		return false;
	}

	function editSavev4_PlaceTypes(id, inList) { 
	
		if($("#v4_PlaceTypesEditForm"+id).valid() == false) {return false;}
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}

		var newData = $("#v4_PlaceTypesEditForm"+id).serializeObject();
		var formData = $("#v4_PlaceTypesEditForm"+id).serialize();

		// update data on server
		var url = window.root + '/cms/p/modules/v4_PlaceTypes/'+
		'v4_PlaceTypes_Save.php?callback=?&keyName=PlaceTypeID&keyValue='+id+'&'+ formData;
		...
		$.ajax({
			...
			success: function(data, status) {
				...
				if (inList==true) {
					refreshv4_PlaceTypesData(id, newData);
					$(".editFrame").hide('slow');
				}
			},
	...
	function deletev4_PlaceTypes(id, inList) { 
		...
		// update data on server
		var url = window.root + '/cms/p/modules/v4_PlaceTypes/'+
		'v4_PlaceTypes_Delete.php?PlaceTypeID='+ id+'&'+formData+'&callback=?';
	...
	function editPrintv4_PlaceTypes(id, inList) {
		// default value. inList znaci je li prikaz sa liste ili nije
		if (notDefined(inList)) {inList=true;}
	...
	function refreshv4_PlaceTypesData(id, newData) {

	  var result = $.grep(data_v4_PlaceTypes, function(e){ return e.PlaceTypeID == id; });
	  ...
	  if (result.length == 0) {
		// not found
		    ...
		    changeall_v4_PlaceTypes(id, ress);

		    data = data_v4_PlaceTypes;

		    var source   = $("#v4_PlaceTypesListTemplate").html();
		    var template = Handlebars.compile(source);

		    var HTML = template({v4_PlaceTypes : data});
	...
	function changeall_v4_PlaceTypes( id, sve ) {
	   for (var i in data_v4_PlaceTypes) {
		 if (data_v4_PlaceTypes[i].PlaceTypeID == id) {
		    data_v4_PlaceTypes[i] = sve;
