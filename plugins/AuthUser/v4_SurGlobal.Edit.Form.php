...
session_start();
// ovo je jebavalo dropdown... mater ti jeben
//require_once '../../../headerScripts.php';
	...
	# Details  red
	if (isset($_REQUEST['OwnerID']) and $_REQUEST['OwnerID'] != '') {
		$dbk = $db->getKeysBy('OwnerID','asc','WHERE OwnerID = ' . $_REQUEST['OwnerID']);

		# Details  red
		if(count($dbk) > 0) $db->getRow($dbk[0]);
	}
	else $db->New_v4_SurGlobal();

	
	// novo pravilo
	//if($db->getOwnerID() == $_REQUEST['OwnerID']) $db->getRow($dbk[0]);

/*

	EDIT GLOBAL SURCHARGES

*/
?>	
<form id="v4_SurGlobalEditForm" class="form " enctype="multipart/form-data" method="post" onsubmit="return false;">	
	<div class="box-body ">
        <div class="row">
			<div class="col-md-12">
						...
						<input type="hidden" name="SiteID" id="SiteID" class="w100" value="1" readonly>
