...
<div class="container-fluid">
	...
	foreach ($subDArray as $SubDriver) { // STUPAC (driver)
		if ($SubDriver>0) $au->getRow($SubDriver);
		else $au->getRow($_SESSION['OwnerID']);
		...
		foreach ($row as $rw) {
			if (($SubDriver>0 && ($rw->SubDriver==$SubDriver || $rw->SubDriver2==$SubDriver || $rw->SubDriver3==$SubDriver)) ||
				($SubDriver==0 && $rw->SubDriver==0))
				$DetailsIDArray[] = $rw->DetailsID;  
		}
		if (count($DetailsIDArray)>0) {
			...
			foreach ($DetailsIDArray as $ID) { // REDAK u STUPCU (transfer)
					...
					if(!empty($od->RouteID) && empty($od->getTransferDuration()) ) {
						$or->getRow($od->getRouteID());
						$od->setTransferDuration($or->getDuration());
						$saveRow = true;
					}	
					...
					/*if(!empty($od->RouteID) ) {
						...
						if ($od->getTransferDuration()==0) $od->setTransferDuration($duration);
					}*/
					// elegantniji FIX :)
					if(!empty($od->getTransferDuration()) ) {
						$duration = $od->getTransferDuration();
					} else $duration = 'N/A';

					// da li flight time u datumskom konfliktu sa pickuptime ili droptime
					$flightTimeConflict=false;
					$op->getRow($od->getPickupID());
					if ($op->getPlaceType()==1 && $od->getSubPickupTime()<$od->getFlightTime()) $flightTimeConflict=true;
				...
				<div class="row white shadow" style="cursor:default; padding:8px !important;background:<?= $bgColor ?>; margin:12px 0">
					...
					<div class="row">
						<div class="row">
							...
							<div class="col-md-3">
								<?
								if ($od->getSubPickupTime()==$od->getPickupTime()) $color2='';
								else $color2='red';
								
								?>
								<input type="text" class="w100 <?= $color2 ?>"  id="PickupTimeX_<?= $i ?>"
									name="PickupTimeX_<?= $i ?>"
									value="<?= $od->getPickupTime();?>" 
								style="font-weight:bold;text-align:center"/>
							</div>
							<!-- info icons -->
